...
namespace QuantConnect.Algorithm.CSharp
{
    public class BrokerageAlgorithmSettings
    {
        ...
        public MarketSymbols MarketSymbols { get; protected set; }
        ...
        {
            _url = new Uri(brokerageSettingsURL);
            LoadConfigs();
            SymbolsToAdd();
        }
        ...
        public async void LoadConfigs()
        {
            ...
            var json = httpClient.GetStringAsync(_url).Result;
        ...
        private void ConfigSymbols()
        {
            ...
            {
                MarketSymbols = new KrakenMarketSymbols();
            }
            else if (Markets.Contains("Binance"))
            {
                MarketSymbols = new BinanceMarketSymbols();
            }
            else if (Markets.Contains("Bybit"))
            {
                MarketSymbols = new BybitMarketSymbols();
            }
            else if (Markets.Contains("Oanda"))
            {
                MarketSymbols = new OandaMarketSymbols();
            }
            else if (Markets.Contains("Bitfinex"))
            {
                MarketSymbols = new BitfinexMarketSymbols();
            }
            else if (Markets.Contains("Coinbase"))
            {
                MarketSymbols = new CoinbaseMarketSymbols();
            }
            else
            {
                MarketSymbols = new DefaultMarketSymbols();
    ...
    public abstract class MarketSymbols
    {
    ...
    public class DefaultMarketSymbols : MarketSymbols
    {
    ...
    public class KrakenMarketSymbols : MarketSymbols
    {
        public override Symbol CryptoSymbol { get; set; } = Symbol.Create("BTCUSD", SecurityType.Crypto, Market.Kraken);
    }

    public class BinanceMarketSymbols : MarketSymbols
    {
    ...
    public class BybitMarketSymbols : MarketSymbols
    {
        public override Symbol CryptoSymbol { get; set; } = Symbol.Create("BTCUSDT", SecurityType.Crypto, Market.Bybit);
        public override Symbol CryptoFutureSymbol { get; set; } = Symbol.Create("BTCUSDT", SecurityType.CryptoFuture, Market.Bybit);
    }

    public class OandaMarketSymbols: MarketSymbols
    {
        public override Symbol ForexSymbol { get; set; } = Symbol.Create("EURUSD", SecurityType.Forex, Market.Oanda);
    }

    public class BitfinexMarketSymbols: MarketSymbols
    {
        public override Symbol CryptoSymbol { get; set; } = Symbol.Create("BTCUSD", SecurityType.Crypto, Market.Bitfinex);
    }

    public class CoinbaseMarketSymbols : MarketSymbols
    {
        public override Symbol CryptoSymbol { get; set; } = Symbol.Create("BTCUSD", SecurityType.Crypto, Market.Coinbase);
    }
}
